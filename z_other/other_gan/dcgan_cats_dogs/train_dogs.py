...
# Setup
dog_dir = r"P:\DataSets\catsvsdogs\training_set\training_set\dogs"  # Changed from cat_dir to dog_dir
img_size = 64
BATCH_SIZE = 64
EPOCHS = 150
NOISE_DIM = 100
SAVE_DIR = "generated_images/dogs"  # Changed folder name for saving images
...
dog_images = load_images_from_folder(dog_dir)  # Changed to load dog images
...
# Training Loop
def train(dataset, generator, discriminator, save_folder):
    g_optimizer = tf.keras.optimizers.RMSprop(5e-5)
    d_optimizer = tf.keras.optimizers.RMSprop(5e-5)

    for epoch in range(EPOCHS):
        d_losses, g_losses = [], []
        print(f"Epoch {epoch + 1}/{EPOCHS} - DOGS")  # Changed "CATS" to "DOGS"
    ...
    generator.save(os.path.join(save_folder, "generator_dog.keras"))  # Changed to "dog"
    discriminator.save(os.path.join(save_folder, "discriminator_dog.keras"))  # Changed to "dog"

# Main
dog_generator = make_generator()
dog_discriminator = make_discriminator()
train(dog_dataset, dog_generator, dog_discriminator, SAVE_DIR)
